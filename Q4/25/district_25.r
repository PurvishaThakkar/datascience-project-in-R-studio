#========================normlized crime in PUNJAB =====================
library(ggplot2)
theme_set(theme_bw())  
library(dplyr)
library(ggplot2)
table=read.csv("E:/project/crimes.csv")
data2=table %>% group_by(DISTRICT) %>% filter(STATE.UT == "PUNJAB") %>% summarise(all_crimes=sum(Rape,Kidnapping.and.Abduction,Dowry.Deaths,Assault.on.women.with.intent.to.outrage.her.modesty,Insult.to.modesty.of.Women,Cruelty.by.Husband.or.his.Relatives,Importation.of.Girls))
#View(data2)
...
ggplot(data,aes(x=DISTRICT,y=avg))+
  ...
  labs(subtitle = "normlized crimes in PUNJAB",
